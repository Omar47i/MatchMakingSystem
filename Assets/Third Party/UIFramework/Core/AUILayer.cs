...
namespace deVoid.UIFramework {
    ...
    public abstract class AUILayer<TScreen> : MonoBehaviour where TScreen : IUIScreenController {
        protected Dictionary<ScreenId, TScreen> registeredScreens;
        ...
        public virtual void Initialize() {
            registeredScreens = new Dictionary<ScreenId, TScreen>();
        }
        ...
        public void RegisterScreen(ScreenId screenId, TScreen controller) {
        ...
        public void UnregisterScreen(ScreenId screenId, TScreen controller) {
            if (registeredScreens.ContainsKey(screenId)) {
        ...
        public void ShowScreenById(ScreenId screenId) {
            TScreen ctl;
        ...
        public void ShowScreenById<TProps>(ScreenId screenId, TProps properties) where TProps : IScreenProperties {
            TScreen ctl;
            if (registeredScreens.TryGetValue(screenId, out ctl)) {
        ...
        public void HideScreenById(ScreenId screenId) {
            TScreen ctl;
            if (registeredScreens.TryGetValue(screenId, out ctl)) {
                HideScreen(ctl);
            }
        ...
        protected virtual void ProcessScreenRegister(ScreenId screenId, TScreen controller) {
            controller.ScreenId = screenId;
            registeredScreens.Add(screenId, controller);
            controller.ScreenDestroyed += OnScreenDestroyed;
        }

        protected virtual void ProcessScreenUnregister(ScreenId screenId, TScreen controller) {
            controller.ScreenDestroyed -= OnScreenDestroyed;
        ...
        private void OnScreenDestroyed(IUIScreenController screen) {
            ...
            //}
            if(registeredScreens.ContainsKey(screen.ScreenId))
                UnregisterScreen(screen.ScreenId, (TScreen)screen);
        }
    }
}
